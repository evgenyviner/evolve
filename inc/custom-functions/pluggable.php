...
if ( ! function_exists( 'evolve_comment_reply' ) ):
	...
	function evolve_comment_reply( $return = false ) {
		...
		$reply_text = apply_filters( 'evolve_reply_text', (string) __( 'Reply', 'evolve' ) ); // Available filter: evolve_reply_text
		$login_text = apply_filters( 'evolve_login_text', (string) __( 'Log in to reply.', 'evolve' ) ); // Available filter: evolve_login_text
...
add_filter( 'wp_get_attachment_link', 'evolve_add_lighbox_rel' );
...
function evolve_replace_reply_link_class( $class ) {
	$class = str_replace( "class='comment-reply-link", "class='btn btn-sm", $class );

	return $class;
}

function evolve_add_lighbox_rel( $attachment_link ) {
