                ...
                <div class="image" aria-haspopup="true">
                    ...
                    <div class="image-extras">
                        <div class="image-extras-content">
                            ...
                            <a style="<?php echo $link_icon_css; ?>" class="icon link-icon"
                               href="<?php echo $permalink; ?>">Permalink</a>
							<?php
							if ( get_post_meta( $post->ID, 'evolve_video_url', true ) ) {
								$full_image[0] = get_post_meta( $post->ID, 'evolve_video_url', true );
							}
							?>
                            <a style="<?php echo $zoom_icon_css; ?>" class="icon gallery-icon"
                               href="<?php echo $full_image[0]; ?>" rel="prettyPhoto[gallery<?php echo $post->ID; ?>]"
                               title="<?php echo get_post_field( 'post_excerpt', get_post_thumbnail_id() ); ?>"><img
                                        style="display:none;"
                                        alt="<?php echo get_post_meta( get_post_thumbnail_id(), '_wp_attachment_image_alt', true ); ?>"/>Gallery</a>
                            <br/>
                            <h3><a href="<?php echo $permalink; ?>"><?php the_title(); ?></a></h3>
                            <h4>
                                <span class="cats"><?php echo get_the_term_list( $post->ID, 'category', '', ', ', '' ); ?></span>
                            </h4>
                        </div>
                    </div>
                </div>
		...
		if ( $smof_data['posts_slideshow'] ):
			...
			while ( $i <= $smof_data['posts_slideshow_number'] ):
				...
				if ( $attachment_id ):
                    ...
                    <li>
                        <div class="image">
                            <a href="<?php the_permalink(); ?>"><img src="<?php echo $attachment_image[0]; ?>"
                                                                     alt="<?php echo $attachment_data['image_meta']['title']; ?>"/></a>
                            <a style="display:none;" href="<?php echo $full_image[0]; ?>"
                               rel="prettyPhoto[gallery<?php echo $post->ID; ?>]"
                               title="<?php echo get_post_field( 'post_excerpt', $attachment_id ); ?>"><img
                                        style="display:none;"
                                        alt="<?php echo get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ); ?>"/></a>
                        </div>
                    </li>
				<?php
				endif;
